...
export default function Delete({id}:{id: number | undefined}){
    return(
        <div className="bg-white w-full md:w-2/5 m-6 h-80 rounded-lg flex flex-col justify-between gap-2 px-6 py-2">
            ...
            <h1 className="text-black font-semibold md:text-lg text-center text-md lg:text-xl xl:text-2xl 2xl:text-3xl 3xl:text-4xl  ">Realmente deseja deletar esse usuário?</h1>
            <div className="flex flex-row w-full justify-between gap-3 mt-4 mb-8">
                ...
                <Link className="w-1/2 x1:w-3/12 2xl:w-2/12" href={"/admin/manage/members"}>
                    <button onClick={()=> deleteMember(id)} style={{backgroundColor: '#0A0953'}} className="font-semibold px-3 md:px-6 text-white rounded-3xl h-6 md:h-7 lg:h-9 xl:h-10  text-center w-full text-md md:text-lg lg:text-xl xl:text-2xl 2xl:text-3xl 3xl:text-4xll">
                        Confirmar
                    </button>
                </Link>
            </div>
